...
def owned_resource_clause(model, user_id: str):
    """
    Scopes resources to a user while allowing unowned legacy rows during local upgrades.

    Production migrations assign legacy rows to the first staff owner before new users
    can access them, so newly created rows are always strictly user-owned.
    """
