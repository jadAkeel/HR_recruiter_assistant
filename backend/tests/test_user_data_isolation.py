...
def test_authenticated_users_share_recruiter_data_and_workflows() -> None:
    app = create_app()
    fixture = Path(__file__).parent / "fixtures" / "cv_engineer.txt"
    cv_text = fixture.read_text(encoding="utf-8")
    marker = uuid.uuid4().hex[:8]
    cv_a = cv_text.replace("Alex Engineer", f"Shared Visibility Alpha {marker}").replace(
        "alex.engineer@example.com", f"shared-alpha-{marker}@example.com"
    ).encode()
    cv_b = cv_text.replace("Alex Engineer", f"Shared Visibility Beta {marker}").replace(
        "alex.engineer@example.com", f"shared-beta-{marker}@example.com"
    ).encode()
    ...
    with TestClient(app) as client:
        ...
        assert {job_a_id, job_b_id} <= jobs_a
        assert {job_a_id, job_b_id} <= jobs_b
        assert {candidate_a_id, candidate_b_id} <= candidates_a
        assert {candidate_a_id, candidate_b_id} <= candidates_b

        assert client.get(f"/api/v1/candidates/{candidate_b_id}", headers=owner_a).status_code == 200
        assert client.patch(
            f"/api/v1/jobs/{job_b_id}",
            json={"title": "Shared frontend role"},
            headers=owner_a,
        ).status_code == 200

        matching = client.post(
            f"/api/v1/jobs/{job_a_id}/match?use_hybrid=false&top_k=100&search=Shared%20Visibility",
            headers=owner_b,
        )
        assert matching.status_code == 200, matching.text
        matching_ids = {row["candidate_id"] for row in matching.json()["results"]}
        assert {candidate_a_id, candidate_b_id} <= matching_ids
        ...
        assert cross_report.status_code == 200, cross_report.text
        assert cross_report.json()["candidates"][0]["candidate_id"] == candidate_b_id

        cross_feedback = client.post(
            "/api/v1/matching/feedback",
            json={"job_id": job_a_id, "candidate_id": candidate_a_id, "skill": "python"},
            headers=owner_b,
        )
        assert cross_feedback.status_code == 200, cross_feedback.text
        feedback_stats = client.get("/api/v1/matching/feedback/stats", headers=owner_a)
        ...
        assert cross_interview.status_code == 200, cross_interview.text

        assert client.delete(f"/api/v1/jobs/{job_b_id}", headers=owner_a).status_code == 200
